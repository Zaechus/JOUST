...
class Player(object):
    def __init__(self, i, pos, size, a, maxV, left_img, right_img, lFlap,
                 rFlap):
        self.otype = "player"
        self.i = i
        self.pos = pos
    ...
    def draw(self):
        imgRect = Rect(screen, backgroundColor,
                       (self.pos[0], self.pos[1], self.size[0], self.size[1]))
...
def DRAW_MENU():
    ...
    mp2rFlap = pg.transform.scale(p2rFlap,
                                  (p2rFlap.get_size()[0] * imageScale,
                                   p2rFlap.get_size()[1] * imageScale))

    imgRect1 = Rect(screen, backgroundColor,
                    (width - p2rFlap.get_size()[0] * imageScale - 15,
                     height - p2rFlap.get_size()[1] * imageScale - 15, 10, 10))
...
def DRAW_GAME():
    ...
    Rect(screen, backgroundColor,
         (10, BBuffer + 10, 150, height - BBuffer - 20))
...
def DRAW_WIN():
    winText = menuFont.render("Player   " + str(whoWins) + "   wins!", 1,
                              (255, 0, 255))
    screen.fill(backgroundColor)
